...
def deinit():
	...
	host.unregisterHandler('TimeLimitReached', onTimeLimitReached, 1)	
	...
	if g_debug: print "gpm_sp.py uninitialized"
...
def onPlayerDeathCQ(victim, vehicle):	
	if victim == None or not victim.isValid():
		return None	
...
def getOccupyingCP(player):
 	if player == None or not player.isValid():
		return None
