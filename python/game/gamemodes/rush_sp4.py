...
class Rush:
	...
	def getCurrentTargetsDown(self):
		return (not self.getCurrentTargetObjectA().isValid() or self.getCurrentTargetObjectA().getIsWreck()) and (not self.getCurrentTargetObjectB().isValid() or self.getCurrentTargetObjectB().getIsWreck()):
	...
	def addC4OnTargetVehicle(self, currentPlayer, targetVehicle):
		if targetVehicle.c4object == None and targetVehicle.occupyingPlayer == currentPlayer and currentPlayer.getTeam() == self.attackTeam and abs(host.timer_getWallTime() - targetVehicle.playerEnterAt - C4_PLACE_DELAY) <=0.1:
			spawnpos = (TARGET_SPAWN_HEIGHT * 0.6, TARGET_SPAWN_HEIGHT * 1.2, 0.0)
	...
	def linkC4ToTargetVehicle(self, targetVehicle):
		...
		for obj in pcos:
			...
			if obj.templateName == C4OBJECTNAME:# and self.getRoughDistance(obj.getPosition(), (0.0, TARGET_SPAWN_HEIGHT * 1.2, 0.0)) < 1:
				...
				c4pos = (targetpos[0] - 0.0, targetpos[1] + 1.2, targetpos[2] + 0.0)
	...
	def disarmC4OnTargetVehicle(self, currentPlayer, targetVehicle):
		...
		if targetVehicle.isValid() and not targetVehicle.getIsWreck() and targetVehicle.occupyingPlayer == currentPlayer and currentPlayer.getTeam() == self.defenseTeam and targetVehicle.c4object != None and abs(host.timer_getWallTime() - targetVehicle.playerEnterAt - C4_DISARM_DELAY) <=0.1:
			...
			MSG_bombdisarm(targetVehicle.getOccupyingPlayers()[0])
			targetVehicle.c4object.setPosition((TARGET_SPAWN_HEIGHT * 0.6, TARGET_SPAWN_HEIGHT * 1.5, 0.0))
		...
		if int(cp.cp_getParam('timeToGetControl')) == rushData.getCurrentTarget():
			...
			if rushData.getCurrentTargetsDown():
				...
				if cp.cp_getParam('team') == 0:
					...
					cp.cp_setParam('takeOverChangePerSecond', -999)
			elif (not cp.targetVehicle.isValid() or cp.targetVehicle.getIsWreck()) and cp.cp_getParam('team') == rushData.getDefenseTeam():
...
def onTargetTrigger(triggerId, target, vehicle, enter, userData):
	...
	if not target.isValid() or target.getIsWreck(): return
	if g_echooutput: host.rcon_invoke("echo debugValidTargetTrigger")
	if vehicle and vehicle.getParent(): return
	
	if target.occupyingPlayer and not target.occupyingPlayer.getDefaultVehicle() in bf2.triggerManager.getObjects(target.triggerId):
		target.occupyingPlayer = None
	...
	if enter:
			...
			for player in playersInVehicle:
					...
					if player.getDefaultVehicle() in bf2.triggerManager.getObjects(target.triggerId):
...
def onPlayerKilledRush(victim, attacker, weapon, assists, object):
	...
	if attacker: 
		target = getOccupyingTarget(attacker)
		if target != None:
			onTargetTrigger(-1, target, attacker.getVehicle(), True, None)
		'''# give defend score if killing enemy within cp radius
		if attacker != None and attacker.getTeam() != victim.getTeam():
...
def onEnterVehicleRush(player, vehicle, freeSoldier = False):
	...
	if target != None:
		onTargetTrigger(-1, target, player.getVehicle(), False, None)
...
def getOccupyingTarget(player):
	...
	if rushData.getCurrentTargetObjectA() and rushData.getCurrentTargetObjectA().isValid() and not rushData.getCurrentTargetObjectA().getIsWreck():
		pcos = bf2.triggerManager.getObjects(rushData.getCurrentTargetObjectA().triggerId)
		for o in pcos:
			if o == player.getDefaultVehicle():
				return rushData.getCurrentTargetObjectA()
	
	if rushData.getCurrentTargetObjectB() and rushData.getCurrentTargetObjectB().isValid() and not rushData.getCurrentTargetObjectB().getIsWreck():
