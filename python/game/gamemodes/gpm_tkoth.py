...
def onGameStatusChanged(status):
	...
	if status == bf2.GameStatus.Playing: 
		...
		if g_debug: print "TKOTH gamemode initialized."
	else:
		bf2.triggerManager.destroyAllTriggers()
		g_controlPoints = []
...
def onCPStatusChange(cp, top):
		...
		for p in scoringPlayers:
			...
			if takeoverType == TAKEOVERTYPE_CAPTURE:
				if p in firstPlayers:
					...
					playerId = p.index
					if not p.isAIPlayer():
						timer.once(0.03, host.rcon_invoke, "sound.playSound FlagCaptureIndication")
					addScore(p, SCORE_CAPTURE, RPL,"cap" ,"x" + str(SCORE_CAPTURE), 0)
				else:
					p.score.cpAssists += 1
					if p.isCommander():
						return
					...
					if not p.isAIPlayer():
						timer.once(0.03, host.rcon_invoke, "sound.playSound FlagCaptureIndication")
					addScore(p, SCORE_CAPTUREASSIST, RPL,"capas" ,"x" + str(SCORE_CAPTUREASSIST), 0)
				...
				if p in firstPlayers:
					...
					addScore(p, SCORE_NEUTRALIZE, RPL,"neu" ,"x" + str(SCORE_NEUTRALIZE), 0)
				else:
					...
					addScore(p, SCORE_NEUTRALIZEASSIST, RPL,"neuas" ,"x" + str(SCORE_NEUTRALIZEASSIST), 0)	
	...
	if enemyTicketLossInstant > 0 and newTeam > 0:
		...
		if newTeam == 1:
			punishedTeam = 2
			ticketLossPerSecTeam1 = 0
			ticketLossPerSecTeam2 = 1
			tickets = bf2.gameLogic.setTicketChangePerSecond(2, -ticketLossPerSecTeam2)
			tickets = bf2.gameLogic.setTicketChangePerSecond(1, -ticketLossPerSecTeam1)
		elif newTeam == 2:
			punishedTeam = 1
			ticketLossPerSecTeam2 = 0
			ticketLossPerSecTeam1 = 1
			tickets = bf2.gameLogic.setTicketChangePerSecond(1, -ticketLossPerSecTeam1)
			tickets = bf2.gameLogic.setTicketChangePerSecond(2, -ticketLossPerSecTeam2)
