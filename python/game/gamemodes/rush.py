...
class Rush:
	...
	def addC4OnTargetVehicle(self, targetVehicle):
		if targetVehicle.getOccupyingPlayers() != None and targetVehicle.getOccupyingPlayers()[0].getTeam() == self.attackTeam and abs(host.timer_getWallTime() - targetVehicle.playerEnterAt - C4_PLACE_DELAY) <=0.1:
			spawnpos = (0.0, TARGET_SPAWN_HEIGHT * 1.2, 0.0)
			self.addObjectOnTeam(C4OBJECTNAME, spawnpos, (0.0, 0.0, 0.0), 0)
	...
	def linkC4ToTargetVehicle(self, targetVehicle):
		...
		pcos = bf2.objectManager.getObjectsOfType('dice.hfe.world.ObjectTemplate.PlayerControlObject')
		if targetVehicle.c4object != None and targetVehicle.getOccupyingPlayers() == None or targetVehicle.getOccupyingPlayers()[0].getTeam() == self.defenseTeam: return
		for obj in pcos:
			if not obj.isValid() or obj.getIsWreck(): continue
			...
			if obj.templateName == C4OBJECTNAME:# and self.getRoughDistance(obj.getPosition(), (0.0, TARGET_SPAWN_HEIGHT * 1.2, 0.0)) < 1:
				...
				c4pos = (targetpos[0] - 0.0, targetpos[1] + 2.5, targetpos[2] + 0.0)
	...
	def disarmC4OnTargetVehicle(self, targetVehicle):
		...
		if targetVehicle.isValid() and not targetVehicle.getIsWreck() and targetVehicle.getOccupyingPlayers() != None and targetVehicle.getOccupyingPlayers()[0].getTeam() == self.defenseTeam and targetVehicle.c4object != None and abs(host.timer_getWallTime() - targetVehicle.playerEnterAt - C4_PLACE_DELAY) <=0.1:
			host.rcon_invoke("echo debugdisarmC4Success")
			if hasattr(obj, 'hasLink') and obj.hasLink == True: obj.hasLink = False
			targetVehicle.c4object.setPosition(TARGET_SPAWN_HEIGHT / 2, TARGET_SPAWN_HEIGHT * 1.5, 0.0)
