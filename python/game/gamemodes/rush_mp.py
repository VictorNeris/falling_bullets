...
DUMMYTARGETNAME_A = 'target_aircontroltower_mec_dummyA'
DUMMYTARGETNAME_B = 'target_aircontroltower_mec_dummyB'
...
class Rush:
	...
	def gameSetting(self, cps):
		...
		for obj in tmpSequence:
			...
			try:
				...
				if int(self.targetSequence[index - 1].cp_getParam('timeToGetControl')) == index:
					self.targetSequence[index - 1] = obj
				else: self.targetSequence[index] = obj
			except:
	...
	def toNextTarget(self):
		if self.getCurrentTargetObjectA() and self.getCurrentTargetObjectA().getIsWreck() and self.getCurrentTargetObjectB() and self.getCurrentTargetObjectB().getIsWreck():
	...
	def addTargetVehicle(self):
		self.addObjectOnTeam(TARGETNAME_A, self.targetSequence[self.currentTarget*2-2].getPosition(), self.targetSequence[self.currentTarget*2-2].getRotation(), self.defenseTeam)
		self.addObjectOnTeam(TARGETNAME_B, self.targetSequence[self.currentTarget*2-1].getPosition(), self.targetSequence[self.currentTarget*2-1].getRotation(), self.defenseTeam)
		
	def linkTargetVehicle(self):
		host.rcon_invoke("echo linkTargetVehicle")
		pcos = bf2.objectManager.getObjectsOfType('dice.hfe.world.ObjectTemplate.PlayerControlObject')
		for obj in pcos:
			if obj.getIsWreck(): continue
		...
		host.rcon_invoke("echo linkTargetVehicledebugA%d" % len(self.targetSequence))
		for obj in self.targetSequence:
			if obj.targetVehicle == None:
				host.rcon_invoke("echo \"No spawning target vehicle on ".join(obj.templateName).join("\""))
...
def onGameStatusChanged(status):
	...
	if status == bf2.GameStatus.Playing: 
		...
		host.registerHandler('VehicleDestroyed', onVehicleDestroyed)#for switching targets
...
def onCPTrigger(triggerId, cp, vehicle, enter, userData):
	...
	#calcTakeOverChangePerSecond(cp)
	#calcTakeOverChangePerSecondRush(cp)
	
def calcTakeOverChangePerSecondRush(cp):
	if cp in rushData.getTargets() and cp.targetVehicle.getIsWreck():
