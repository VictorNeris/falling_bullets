...
def deinit():
	...
	if g_debug: 
		log.deinit()
		print "rush_v2.py uninitialized"
...
def calcTakeOverChangePerSecondRush(cp):
	#if g_echooutput: log.output("debugcalcTakeOverChangePerSecondRush")
	...
	if cp in rushData.getTargets():
		if int(cp.cp_getParam('timeToGetControl')) == rushData.getCurrentTarget():
			#if g_echooutput: log.output("debugCurrentTarget")
			if rushData.getCurrentTargetsDown():
				if g_echooutput: log.output("debugCurrentTargetsDown")
				if cp.cp_getParam('team') == 0:
					if g_echooutput: log.output("debugcalcTakeOverChangePerSecondRushAttackTeamGetFlag")
					...
					if g_echooutput: log.output("debugcalcTakeOverChangePerSecondRushDefenseTeamLostFlag")
					cp.cp_setParam('takeOverChangePerSecond', -999)
				...
				if g_echooutput: log.output("debugcalcTakeOverChangePerSecondRushDefenseTeamLostFlag2")
				cp.cp_setParam('takeOverChangePerSecond', -999)
		'''elif int(cp.cp_getParam('timeToGetControl')) < rushData.getCurrentTarget():
			if cp.cp_getParam('team') == 0:
				log.output("debugcalcTakeOverChangePerSecondRushAttackTeamGetFlagLate")
				cp.cp_setParam('flag', rushData.getAttackTeam())
				cp.cp_setParam('takeOverChangePerSecond', 999)
			elif cp.cp_getParam('team') == rushData.getDefenseTeam():
				log.output("debugcalcTakeOverChangePerSecondRushDefenseTeamLostFlagLate")
				cp.cp_setParam('takeOverChangePerSecond', -999)'''

def setCpTeam(cp, team):
	if cp.cp_getParam('team') == 0:
...
def enterTargetRange(target, player):	
	if g_echooutput: log.output("debugenterTargetRange")
...
def exitTargetRange(target, player):	
	if g_echooutput: log.output("debugexitTargetRange")
...
def onTargetTrigger(triggerId, target, vehicle, enter, userData):
	if g_echooutput: log.output("debugonTargetTrigger")
	if not target.isValid() or target.destroyed: 
		if g_echooutput: log.output("debugValidTargetTrigger")
		return
	if vehicle and vehicle.getParent(): return
...
def onVehicleDestroyedRush(vehicle, attacker):
	if vehicle.templateName == TARGETNAME_A :
		...
		log.output("debugVehicleDestroyed")
...
def rcon_debugoutput(self, ctx, cmd):
	...
	if rushData.Adestroyed: log.output("debugrushDataADestroyed") 
	...
	if rushData.Bdestroyed: log.output("debugrushDataBDestroyed") 
...
'''def rcon_feedback(self, ctx, cmd):
	host.rcon_invoke('echo feedback')
