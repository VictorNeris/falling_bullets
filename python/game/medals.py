...
# this is the central part of the indication system, for all the indications are following the scoring event here. 
# ------------------------------------------------------------------------------
import time
import host
...
from game.scoringCommon import addCommanderScore

from constants import *
from statistics import http_backend_addr, http_backend_port, medals_custom_data
from miniclient import miniclient, http_get
...
if (medals_custom_data != ''):
	try:
		exec 'from medal_data_' + str(medals_custom_data) + ' import *'
	except:
...
def onPlayerKilled3(victim, attacker, weapon, assists, object):
	if attacker != None and not attacker.isAIPlayer():
		checkMedals(attacker)
	
	
	
def onExitVehicle(player, vehicle):
	if player != None and not player.isAIPlayer():
		checkMedals(player)
...
def onPlayerScore(player, difference):
	...
	if not player.isCommander():
		addCommanderScore(player, difference)
